...
- **📂 从会话恢复** — 右键任意灯泡 →「从会话打开」浏览最近的 opencode 会话（从本地 SQLite 数据库读取）。点击任意会话即可在新终端中恢复 — 自动 `cd` 到项目目录并执行 `opencode -s <session_id>`，完整还原对话历史。
...
右键任意灯泡 → **自定义图标** 打开设置面板。每个状态（运行中 / 需人工介入 / 已完成）都有独立的配置卡片，你可以：

- **拖放** PNG、JPG 或 GIF 动画到拖拽区
- **点击**拖拽区浏览并选择文件
- **实时预览**图标效果（GIF 会在预览中自动播放）
- **恢复默认**还原为内置灯泡

自定义图标保存在 `~/.config/opencode-traffic-light/icons/{red,yellow,green}/`。
...
- [x] 脉冲动画 + 悬停提示 + 可拖拽
- [x] 系统托盘图标（聚合状态 + 显示/隐藏浮窗 + 退出）
- [x] 图标尺寸选择（小 / 中 / 大）
- [x] 从会话恢复（浏览最近会话，点击即在新终端恢复对话）
